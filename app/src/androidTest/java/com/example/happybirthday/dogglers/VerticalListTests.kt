...
import com.example.happybirthday.dogglers.view.VerticalListActivity
...
class VerticalListTests : BaseTest() {
  ...
  fun `vertical_scrolling`() {
    ...
    Espresso.onView(ViewMatchers.withText("Faye"))
      .check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
  }

  @Test
  fun `recycler_view_item_count`() {
    Espresso.onView(withId(R.id.vertical_recycler_view)).check(hasItemCount(6))
  }
}
